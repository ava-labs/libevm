...
func (p precompile) run(env vm.PrecompileEnvironment, input []byte) (ret []byte, retErr error) {
	defer func() {
		if r := new(vm.RevertError); errors.As(retErr, r) {
			ret = r.Bytes()
			retErr = vm.ErrExecutionReverted
		}
	}()
	...
	return dispatchers[selector](p.impl, env, input)
}
