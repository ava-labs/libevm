...
func (f *Filter) blockLogs(ctx context.Context, header *types.Header) ([]*types.Log, error) {
	if bloomFilter(getBloomFromHeader(header, f.sys.backend), f.addresses, f.topics) {
...
func (f *Filter) pendingLogs() []*types.Log {
	...
	if bloomFilter(getBloomFromHeader(block.Header(), f.sys.backend), f.addresses, f.topics) {
