...
func TestNewStatefulPrecompile(t *testing.T) {
	...
	run := func(env vm.PrecompileEnvironment, input []byte, suppliedGas uint64) ([]byte, uint64, error) {
		var wantSDBErr error
		if env.ReadOnly() {
			wantSDBErr = vm.ErrWriteProtection
		}
		if _, got := env.StateDB(); got != wantSDBErr {
			return nil, 0, fmt.Errorf("PrecompileEnvironment().StateDB() got error %v; want %v when ReadOnly == %t", got, wantSDBErr, env.ReadOnly())
		}
